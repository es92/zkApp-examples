...
function Initialize({ onReady, mockEffects }: { onReady: OnReady, mockEffects: boolean }) {
  ...
  let [state, setState] = useState({
    stages: {
      loadSnarkyJS: { stage: Stage.NotStarted, time: null } as { stage: Stage, time: null | number },
      makeOrGetKey: { stage: Stage.NotStarted, time: null } as { stage: Stage, time: null | number },
      checkAccountFunded: { stage: Stage.NotStarted, time: null } as { stage: Stage, time: null | number },
      waitForAccountFunded: { stage: Stage.NotStarted, time: null } as { stage: Stage, time: null | number },
      compileZKApp: { stage: Stage.NotStarted, time: null } as { stage: Stage, time: null | number },
      fetchState: { stage: Stage.NotStarted, time: null } as { stage: Stage, time: null | number },
    },
  ...
  useMakeStage('makeOrGetKey', 'checkAccountFunded', async (state) => {
    let response = mockEffects ? { error: null } : await fetchAccount({ publicKey: state.publicKey! });
  ...
  const makeOrGetKey = getStageComponent(
                        'makeOrGetKey', 
                        'Getting Key', 
                        () => {
                          return (<div> { state.madeKey ? "Created new key: " : "Found key: " } 
  ...
  const firstCheck = getStageComponent(
                      ...
                      () => {
                        var faucet_link = 'https://faucet.minaprotocol.com/?address=' + state.publicKey!.toBase58();
                        let accoutState;
                        if (state.firstCheckFoundKey) {
