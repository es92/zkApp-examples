...
let createUseMakeStage = <T extends StateWithStages>(
  ...
  const useMakeStage = (
    priorStageName: keyof typeof state.stages | null,
    ...
    useEffect(
      () => {
        (async () => {
          if (
            state.stages[stageName].stage == Stage.NotStarted &&
            (priorStageName == null ||
              state.stages[priorStageName].stage == Stage.Done)
          ) {
...
let createGetStageComponent = <T extends StateWithStages>(state: T) => {
  let getStageComponent = (
    stage: string,
    text: string,
    ifDone: () => ReactElement | null
  ) => {
    var component = null;
    if (state.stages[stage].stage == Stage.Started) {
      ...
      component = (
        <div>
          <div>
            {" "}
            {text}... done ({state.stages[stage].time}ms){" "}
          </div>
          {ifDone()}
        </div>
      );
